...
import { resetUser, selectLoggedIn, selectUser, setUser } from '../slices/userSlice';
    ...
    <header className='flex flex-col lg:flex-row text-md px-5 py-5 lg:px-20 border-b-2 border-slate-200 items-center min-w-full sticky top-0 bg-white z-10'>
      ...
      <div className='relative flex-1 items-center overflow-hidden rounded-3xl'>
        ...
        <div className='absolute h-full right-0 top-1/2 transform -translate-x-1/2 -translate-y-1/2'>
          <a href="/" className='relative flex h-full before:absolute before:bg-primary before:h-full before:w-20 before:-z-10 before:-right-11'>
      ...
      </div>
      {
        loggedIn ?
          <div className='flex flex-col items-center justify-center space-x-8 lg:flex-row ml-4 lg:ml-10 xl:ml-20'>
            <p><NavLink to="/" className={({ isActive }) => (isActive ? `text-primary hover:text-primary` : `hover:text-primary`)}>Browse</NavLink></p>
            <p><NavLink to="/forum" className={({ isActive }) => (isActive ? `text-primary hover:text-primary` : `hover:text-primary`)}>Forum</NavLink></p>
            <p><NavLink to="/compose" className={({ isActive }) => (isActive ? `text-primary hover:text-primary` : `hover:text-primary`)}>Write Review</NavLink></p>
            <p><NavLink to={`/user/${currentUser?.uid}/dashboard`} className={({ isActive }) => (isActive ? `text-primary hover:text-primary` : `hover:text-primary`)}>Dashboard</NavLink></p>
            <div className='w-40'>
              <p className='hover:cursor-pointer border-2 border-primary bg-primary text-white p-2 px-4 rounded-full text-sm font-bold font-body text-center overflow-ellipsis whitespace-nowrap overflow-hidden'><span className=''>{currentUser.isAnonymous ? "Anonymous" : currentUser.userName ? currentUser.userName : currentUser.email}</span></p>
            </div>
            <div className='relative group'>
              ...
              <div className='absolute peer-hover:block hover:block hidden w-32 border-0 border-slate-100 bg-slate-100 rounded-md shadow-md overflow-hidden'>
                <ul>
                  <li className='hover:bg-primary hover:text-white hover:cursor-pointer'><a className='p-2 block w-full h-full' href={`${!currentUser?.isAnonymous ? `/user/${currentUser?.uid}/profile`: `/`}`}>Profile</a></li>
                  {/* <li className='p-2 hover:bg-primary hover:text-white hover:cursor-pointer'>Dashboard</li> */}
                  <li  onClick={() => signOutUser()} className='p-2 hover:bg-primary hover:text-white hover:cursor-pointer'>Logout</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className='flex flex-col items-center justify-center space-x-8 lg:flex-row ml-4 lg:ml-10 xl:ml-20'>
            ...
            <div className='relative group'>
              ...
              <div className='absolute peer-hover:block hover:block hidden w-32 border-0 border-slate-100 bg-slate-100 rounded-md shadow-md overflow-hidden'>
                <ul>
                  <li className='p-2 hover:bg-primary hover:text-white hover:cursor-pointer'><Link className='w-full h-full' to={`login-signup`}>Profile</Link></li>
                  {/* <li className='p-2 hover:bg-primary hover:text-white hover:cursor-pointer'>Dashboard</li> */}
                  <li className='hover:bg-primary hover:text-white hover:cursor-pointer'><NavLink to="/login-signup"><p className='p-2'>Login/Sign Up</p></NavLink></li>
                </ul>
              </div>
            </div>
          </div>
