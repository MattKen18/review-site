...
import { addHelpful, addReviewToUserSaved, addUnHelpful, getReviewFromFirestore, getUserFromFirestore, getUserHelpfulsAndUnhelpfuls, removeHelpful, removeReviewFromUserSaved, removeReviewToUserSaved, removeUnHelpful } from '../firebase'
import { current } from '@reduxjs/toolkit'
import ReviewPlaceholder from './ReviewPlaceholder'



const Review = ({reviewId}) => {//{review: {id, author, headline, body, genre: {title, color}, tag, images, timestamp, comments, rating}}) => {
  ...
  useEffect(() => {
    getReviewFromFirestore(reviewId).then((review) => {
  ...
  useEffect(() => {
    if (currentAuthor) {
      if (currentAuthor?.saves.indexOf(reviewId) !== -1) {// if the currently logged in user has this review in their saved
        setBookmarked(true)
      }
      if (!currentAuthor?.helpfulReviews) {
        setHelpful(null)
      } else if (currentAuthor?.helpfulReviews.indexOf(reviewId) !== -1) {// if the currently logged in user has this review as helpful
        setHelpful(true)
      } else if (currentAuthor?.unhelpfulReviews.indexOf(reviewId) !== -1) {
        setHelpful(false)
      } else {
        setHelpful(null)
      }
  ...
  const toggleReviewSaved = () => {
    if (bookmarked) { //already saved
      setBookmarked(false)
      removeReviewFromUserSaved(reviewId, currentUser?.uid)
    } else {
      setBookmarked(true)
      addReviewToUserSaved(reviewId, currentUser?.uid)
    }
  }
  ...
  const handleHelpfulClick = () => {
    if (helpful === null) {
      try {
        addHelpful(currentAuthor.uid, reviewId)
        setHelpful(true)
      } catch (e) {
        console.log(e)
      }
      ...
      try {
        removeUnHelpful(currentAuthor.uid, reviewId)
        addHelpful(currentAuthor.uid, reviewId)
        setHelpful(true)
      } catch (e) {
        console.log(e)
      ...
      removeHelpful(currentAuthor.uid, reviewId)
      setHelpful(null)
    }
  } 
  ...
  const handleUnHelpfulClick = () => {
    if (helpful === null) {
      try {
        addUnHelpful(currentAuthor.uid, reviewId)
        setHelpful(false)
      } catch (e) {
        console.log(e)
      }
  ...
  useEffect(() => {
    ...
    try {
      document.getElementById(`${reviewId}-review-detail-click`).style.backgroundColor = review?.genre?.color
    } catch {
  ...
  const reviewHeading = review?.tag ? 
    <p className='text-sm font-light'>{review?.tag}<span className='opacity-70 font-extralight'> • {review?.genre?.title}</span></p> :
    <p className='text-sm font-light'><span className='opacity-70 font-extralight'>{review?.genre?.title}</span></p>
      ...
      <>
        {
          ...
          <>
            {!review?.images.length > 0 ? 
              <>
                <div id={`${reviewId}-review`} className={`relative z-0 flex flex-col min-h-[200px] h-fit bg-white rounded-md shadow-sm mb-10 p-6 overflow-hidden`}>
                  <div id={`${reviewId}-review-detail-click`} className={`group absolute -top-20 -right-20 w-36 h-28 z-1 rotate-45 duration-300 hover:scale-150 hover:cursor-pointer`}>
                    <Link to={`/review/${reviewId}`} className='relative w-full h-full block'>
                  ...
                  <div className='mb-5'>
                    ...
                    <div className='flex space-x-8 items-center'>
                      <h1 className='font-extrabold text-2xl line-clamp-2'><a href={`/review/${reviewId}`} className=''>{review?.headline}</a></h1>
                  ...
                  <div className='relative flex'>
                    <div className='flex items-center space-x-3 text-sm'>
                      <p className=''><span className='p-1 border-2 border-slate-200 rounded-md bg-slate-200 text-xs mr-2 opacity-80'>{timePassed()}</span> By <Link to={`/user/${review?.author?.uid}/profile`} className='font-body underline underline-offset-4 hover:cursor-pointer hover:bg-cyan-100'>{review?.author?.userName || "Anonymous"}</Link></p>
                      <a href={`/review/${reviewId}`} className='flex items-center space-x-1 hover:cursor-pointer hover:opacity-1 hover:text-primary'><ChatBubbleLeftEllipsisIcon className='w-6' /><span className=''>{review?.comments.length > 0 && review?.comments.length}</span></a>
                      {
                    ...
                    <div className='flex-1 space-x-1 items-center'>
                      <div className='relative float-right flex space-x-1 items-center'>
                        {/* <p className='absolute text-xs w-24 -right-1/3 -top-4 hidden peer-hover:block'>Was this helpful?</p> */}
                          {
                            currentUser?.uid !== review?.author?.uid &&
                            <div className='group flex items-center justify-center space-x-1'>
                              <p className='text-xs hidden group-hover:block mr-2 opacity-70'>Was this helpful?</p> 
                              {
                                  !currentUser?.isAnonymous && currentUser ?
                                  <>
                                    <ThumbUpOutlinedIcon id={`thumbs-up-${reviewId}`} className={`peer w-6 hover:cursor-pointer hover:scale-110 ${helpful === true && `text-primary`}`} onClick={() => handleHelpfulClick()} />
                                    <ThumbDownOutlinedIcon id={`thumbs-down-${reviewId}`} className={`peer w-6 hover:cursor-pointer hover:scale-110 ${helpful === false && `text-primary`}`} onClick={() => handleUnHelpfulClick()} />
                                  </>
                                  :
                                  <>
                                    <ThumbUpOutlinedIcon id={`thumbs-up-${reviewId}`} className={`peer w-6 hover:cursor-not-allowed hover:scale-110 ${helpful === true && `text-primary`}`} />
                                    <ThumbDownOutlinedIcon id={`thumbs-down-${reviewId}`} className={`peer w-6 hover:cursor-not-allowed hover:scale-110 ${helpful === false && `text-primary`}`} />
                                  </> 
                                }
                            </div>                
                          }
